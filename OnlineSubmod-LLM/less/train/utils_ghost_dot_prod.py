...
def check_batch(batch):
    all_masked = (batch["labels"] == -100).all(dim=1)  # shape: (batch_size,)

    # Print or log problematic indices
    if all_masked.any():
        raise "INVALID BATCH"


def compute_GradProd_onlinesubmod(model, device, batch_train, validation_loader, optimizer, trainable_layers, 
                               batch_val=None, grads_topk=-1, use_cosine=True, args=None):

    # Get first batch from validation loader
    batch_val = next(iter(validation_loader))
    # check_batch(batch_train)
    # check_batch(batch_val)
    ...
    gradmask_att_based = args["gradmask_att_based"]
    if(gradmask_att_based): 
    ...
    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        outputs = model(**combined_inputs)
        # logits = forward_chunked(model, combined_inputs, chunk_size=4) 
        logits = outputs.logits  # Shape: [batch_size, seq_len, vocab_size]
        # Compute per-sample losses manually
        loss_fct = torch.nn.CrossEntropyLoss(reduction='none')  # Use 'none' to get per-sample losses
        # Reshape logits and labels for loss computation
        ...
        # per_sample_loss, loss = loss_chunked(
        #     model,
        #     combined_inputs,
        #     combined_labels,
        #     combined_inputs.get("attention_mask"),
        #     chunk_size=1,
        # )
        
        # Mask out padding tokens (where labels == -100)
        if(gradmask_att_based):
            mask = combined_inputs["attention_mask"][..., 1:]
        else:
            mask = (shift_labels != -100).float()
        # Get per-sample loss by averaging over sequence length
        loss = (per_position_loss * mask).sum(dim=-1) / mask.sum(dim=-1).clamp(min=1)
    
    pre_acts = [layer.pre_activation for layer in trainable_layers]
    count = 0
    for pa in pre_acts:
         count += 1 if pa.requires_grad else 0
    print("counts", count, len(pre_acts))
        
    assert all(pa.requires_grad for pa in pre_acts), "still detached!"
    ...
    for (dLdZ, a), (dLdZ_val, a_val) in zip(dLdZ_a_train_lst, dLdZ_a_val_lst):
        ...
        dot_prod, cos_sim = grad_dotprod(dLdZ, a, dLdZ_val, a_val, return_cos=True)
        grad_dotproduct_score += to_np(dot_prod)
        grad_cosine_score += to_np(cos_sim)
        del dot_prod, cos_sim

        dot_prod, cos_sim = grad_dotprod(dLdZ, a, dLdZ, a, return_cos=True)
        similarity_local_score += to_np(dot_prod)
        similarity_local_cos_score += to_np(cos_sim)
        del dot_prod, cos_sim
        
        dot_prod, cos_sim = grad_dotprod(dLdZ_val, a_val, dLdZ_val, a_val, return_cos=True)
...
# def old_compute_GradProd_onlinesubmod(model, device, batch_train, validation_loader, optimizer, trainable_layers, 
#                                batch_val=None, grads_topk=-1, use_cosine=True, args=None):
...
#     logits = outputs.logits  # Shape: [batch_size, seq_len, vocab_size]
#     ######
#     # GOES OOM JUST AFTER FORWARD PASS
#     ######
#     # Compute per-sample losses manually
#     loss_fct = torch.nn.CrossEntropyLoss(reduction='none')  # Use 'none' to get per-sample losses
#     # Reshape logits and labels for loss computation
...
def grad_dotprod(A1, B1, A2, B2, return_cos=False) -> torch.Tensor:
    """Compute gradient sample norm for the weight matrix in a linear layer."""
    if A1.dim() == 2 and B1.dim() == 2:
        return grad_dotprod_non_sequential(A1, B1, A2, B2, return_cos=return_cos)
    elif A1.dim() == 3 and B1.dim() == 3:
        return grad_dotprod_sequential(A1, B1, A2, B2, return_cos=return_cos)
    else:
        raise ValueError(f"Unexpected input shape: {A1.size()}, grad_output shape: {B1.size()}")


def grad_dotprod_non_sequential(A1, B1, A2, B2, return_cos=False):
...
def grad_dotprod_sequential(A1, B1, A2, B2, chunk_size=1024, return_cos=False):
